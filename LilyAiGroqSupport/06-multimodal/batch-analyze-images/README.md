# Analyze and Tag Images with a Groq Vision Model on Groq Batch
...
Inside, we can see a function `createJsonlFile`, which creates an object filled with lines of JSON. Each JSON line corresponds to a [chat completion request](https://console.groq.com/docs/text-chat). We're using a [Groq vision model](https://console.groq.com/docs/models) (set via the `GROQ_VISION_MODEL` environment variable) because it has intelligent vision capabilities, which is necessary for analyzing images.
...
This prompt is fairly simple, but it works well with a capable Groq vision model. You can see how easy it is to extract more information from the images - just add more to the prompt.
