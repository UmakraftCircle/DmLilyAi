  ...
  "targets": [
    {
      "name": "free-model",
      "virtual_key": "groq-virtual-key",
      "override_params": {"model": "YOUR_GROQ_MODEL_A"},
    },
     {
      "name": "paid-model",
      "virtual_key": "groq-virtual-key",
      "override_params": {"model": "YOUR_GROQ_MODEL_B"},
    },
  ]
}
...
This configuration routes requests to one Groq model for `paid` and to another Groq model for `free`, based on the `user_plan` metadata.
...
This configuration distributes 70% of traffic to one Groq model and 30% to another, both hosted on Groq.
