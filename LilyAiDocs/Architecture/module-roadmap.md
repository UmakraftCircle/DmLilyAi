...
## Status - 6 intelligence domains + LilyAiCore audited
...
| **LilyAiMain** (Entry) | ⬜ Not yet audited | Contains `Api`, `Discord` (Client/Router/Middleware/Session), `Interaction` (Feedback/Forms/Onboarding/Polls/Workflows). Touched piecemeal while building `remind_me` and the web card this session, never swept. |
...
## Repeatable audit process (used for all 6 domains + LilyAiCore - reuse for LilyAiMain/Frontend next)
