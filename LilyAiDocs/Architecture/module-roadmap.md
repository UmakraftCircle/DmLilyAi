...
| **LilyAiLearning** | ⬜ Not yet audited | Known-real pieces so far (found while auditing others): `Evaluation/model_scan.py` (scheduled, real), `RagLearning/rag_learning.py` (real, feeds Rag's promotion). Unreviewed: `MemoryLearning`, `ToolLearning`, `WebLearning`, `ContextLearning`, `Evaluation/evaluator.py`, `Feedback/`. |

## Build order from here

1. **`LilyAiLearning`** - the one domain with zero audit coverage. It ties together everything found in
   the other 5 (model-scan -> Tool/Core, rag_learning -> Rag, feedback -> Discord/API), so it's the
   natural next step to complete the full sweep.
2. **Close `LilyAiTool`'s open item** - decide `DataTools/`: build it for real, or drop it from the README
   like `KnowledgeMemory` was dropped from code.
3. **Close `LilyAiWeb`'s open item** - confirm `Cache/` is wired to `WebService`, or flag it as another gap.
4. **Docs alignment pass** - once all 6 are audited, update `README.md` and `Architecture/overview.md` in
   ...
   `remind_me` actuator, model-scan seconds override).

## Repeatable audit process (used for Context/Memory/Rag, use for what's left)
...
5. Compare the directory against the README's documented structure; note gaps, don't silently "fix" the
   docs or the code without flagging the gap first.
6. Report findings, get a decision, then act - never restructure a domain without that go-ahead.
